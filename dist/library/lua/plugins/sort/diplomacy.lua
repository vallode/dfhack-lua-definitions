-- THIS FILE WAS GENERATED AUTOMATICALLY. DO NOT EDIT.
---@meta

---@class diplomacy
---@field DiplomacyOverlay diplomacy.DiplomacyOverlay
---@field PreferenceOverlay diplomacy.PreferenceOverlay
local diplomacy

function DiplomacyOverlay:init() end

function DiplomacyOverlay:get_key() end

function DiplomacyOverlay:get_search_key(hfid) end

function DiplomacyOverlay:get_sort() end

function DiplomacyOverlay:preUpdateLayout(parent_rect) end

function PreferenceOverlay:init() end

function PreferenceOverlay:preUpdateLayout(parent_rect) end

function PreferenceOverlay:onRenderFrame(dc, rect) end

return diplomacy
